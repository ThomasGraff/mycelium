<template>
  <div class="results">
    <ResultBox
      ...
      :key="index"
      :result="result"
      @submit-form="submitForm(index, $event)"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    submitForm (index, updatedResult) {
      ...
      this.$emit('submit-form', index, updatedResult)
    }
  }
}
</script>
...
<style scoped>
.results {
  margin-top: 10px; /* Ajustez selon la hauteur de votre NavBar */
  ...
  align-items: center; /* Centre les résultats */
}
</style>
